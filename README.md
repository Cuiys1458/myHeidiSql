...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-283%20passing-success" alt="283 tests passing">
...
- ✅ 真二进制 BLOB（图片等）自动从 WHERE 排除，改这类列时拒绝提交（PRD §A）
- ✅ **BLOB-as-JSON / TEXT-as-JSON 可编辑**：BLOB / VARBINARY / TEXT 列若内容是合法 JSON，按 JSON 处理（双击进 JSON 编辑器；commit 时写真实字符串字面量）
- ✅ 编辑后状态栏显示具体 MySQL 错误，dirty 标记保留（不丢用户输入）

#### JSON 专用编辑器（v0.1.1+）

双击 JSON 列、JSON 类型 BLOB、或内容是 JSON 的 TEXT 列时，弹出 720×520 专用 sheet：

- ✅ NSTextView + 等宽 SF Mono + 行号 gutter
- ✅ 手写 tokenizer 语法高亮（key 蓝 / string 红 / number 紫 / bool/null 橙 / bracket 灰）
- ✅ 250ms debounce 实时校验，错误位置红色波浪线 + byte offset 提示
- ✅ 工具栏：**Format**（2 空格缩进 + sortedKeys）/ **Minify** / **Validate** / Set NULL
- ✅ Apply 前必须 `.valid` 才能保存
- ✅ 底部状态栏：当前顶层 keys 数 + 字节数
...
| **大二进制 BLOB / GEOMETRY 列** | 真二进制（图片 / GIS）只读显示 `[BLOB N bytes]`；BLOB-as-JSON 已可编辑（v0.1.1+） | 仅图片这类改不动 |
...
# 跑测试（核心 283 个（含 BLOB-as-JSON），跳过需要 MySQL 的集成测试）
...
```
✔ Test run with 283 tests passed
```
...
BDD 场景见 `features/*.feature`（14 个 .feature 文件，≈ 1290 行 Gherkin）。
...
_v0.1 · 283 unit tests passing · 12 MB Universal Binary · macOS 14+_
